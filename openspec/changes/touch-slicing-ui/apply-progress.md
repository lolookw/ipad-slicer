...
## Offline PWA — pr7 partial (direct implementation, not SDD, Claude, 2026-09-23)

- Tasks 11.1–11.5 and 11.7 done. Task 11.6 (DiagnosticsSheet expansion) was out of this work unit's scope and stays open. Task 11.8 stays open: two of its e2e scenarios could not be run (see below); the rest were added and pass. Task 11.9 is human/remote.
- `src/pwa/precache.ts`: pure, content-hashed `buildPrecacheManifest()`. Lazy (not precached): `engine/**`, `catalog/**` printer packs (but not `catalog/index.json`/`custom-base.json`), the `GcodePreview-*.js` chunk, `sw.js` itself, `_headers`, `LICENSE`, `*.map`.
- `src/pwa/sw-runtime.ts`: pure routing/cache logic — `shouldHandle` (same-origin GET, no Range), `withIsolationHeaders` (re-stamps COOP/COEP/CORP/nosniff), `classifyRequest`, `installShellVersioned` (atomic: any failed fetch deletes the partial new cache and rethrows, so an interrupted install never replaces the previous complete version), `activateCleanup`, `verifiesPackHash` (checks a catalog pack's content against its filename's sha8 before caching it — the client's own SHA-256 check against the full hash in `index.json` stays the fail-closed gate).
- `src/pwa/sw.ts`: the real service worker; wires the above to `self`/`caches`/`fetch`. Non-DOM SW-only globals (`skipWaiting`, `clients`, `registration`) are typed locally and cast once, rather than adding the conflicting `webworker` lib project-wide. Caches: `ipad-slicer-app-<buildId>` (shell, cache-first, rotated on every deploy), `ipad-slicer-engine` and `ipad-slicer-catalog` (stable names; kept indefinitely since their entries are keyed by content-addressed/immutable URLs). `SW_KILL=1 npm run build` swaps in a second, much smaller SW variant (verified 701 bytes vs. 8119 bytes for the normal build) that clears every `ipad-slicer-*` cache and unregisters; dead-code elimination confirmed the two variants never share branches.
- `src/pwa/vite-sw-plugin.ts`: `closeBundle` hook walks the finished `dist/`, builds the manifest, then runs a second, config-free `vite build()` (library/IIFE, single file) to emit `dist/sw.js` with `BUILD_ID`/precache list/kill-switch flag injected via `define`. Verified against the real build: `catalog/index.json`, `custom-base.json`, `manifest.webmanifest`, `engine-manifest.json`, all icons, `index.html` and every JS/CSS chunk land in the precache list; `engine/**`, printer packs and `GcodePreview-*.js` do not.
- `src/pwa/register.ts` + `UpdateToast.tsx`: registers `/sw.js`; checks for updates on launch and `visibilitychange`; `createUpdateGate` defers `applyUpdate()` while `result.state.status === 'slicing'` or `finishingPreviousSlice` (soft-canceled st) and auto-applies the moment `notifyIdle()` sees idle. **Bug found and fixed during verification:** `controllerchange` fires on the very first `clients.claim()`, not just after our own skip-waiting — an unconditional `location.reload()` there caused an involuntary reload on every fresh visit (this is what first made offline e2e reloads hang/error). Fixed by only arming the reload right before `postSkipWaiting()` runs. `?sw-kill=1` is the runtime escape hatch (unregisters + clears `ipad-slicer-*` caches, registers nothing).
- `src/app/stores/connectivity.ts` + wiring in `AppProvider.tsx`/`App.tsx`: `window` online/offline events drive a visible offline banner independent of engine/slice error state; `navigator.storage.persist()` is requested once after the first successful G-code save or `appinstalled`. `configuration.ts`'s pack-fetch failure now sets `errorKind: 'offlinePack'` (new i18n key, EN/ES) instead of the generic `profileUnavailable` when `navigator.onLine === false`, so an uncached printer pack offline reads as "needs a connection" rather than a crash. `src/catalog/pack-client.ts` itself was not touched, per instruction.
- `public/manifest.webmanifest`, `index.html`, `public/_headers`: added `id`, `description`, `orientation: any`; hand-rolled 192/512/512-maskable/apple-touch-icon PNGs (Node's built-in `zlib.deflateSync`/`crc32` only, no new dependency) since only `icons/icon.svg` existed before — **these are flat-color placeholders (bg `#111827`, a centered rounded square in `#1a64f0`), not final branded art; flagging for design follow-up**. Two `theme-color` metas by `prefers-color-scheme` plus a fallback. `_headers`: `/assets/*` immutable; `/sw.js`, `/index.html`, `/manifest.webmanifest`, `/engine-manifest.json` `no-cache`; existing engine/catalog-pack immutable rules untouched.

### Work Unit Evidence

| Evidence | Observed result |
|---|---|
| Focused tests | `npx vitest run src/pwa`: exit 0, 6 files / 48 tests passed (routing, isolation headers, install atomicity, activation cleanup, catalog-pack hash verification, precache manifest content-addressing, update gate, kill-switch query, the plugin's real nested build). |
| Full regression | `npx vitest run`: exit 0, 53 files / 441 tests passed (up from 437 before this work; only the three pre-existing jsdom canvas warnings). |
| Typecheck | `npx tsc --noEmit`: exit 0. |
| Build | `npm run build`: exit 0; `dist/sw.js` inspected directly — correct `BUILD_ID`, correct precache list, kill-switch branch absent. `SW_KILL=1 npx vite build`: exit 0; kill-switch branch present, normal-path code absent. |
| `npm audit` | 0 vulnerabilities. |
| Real cache contents | A throwaway Playwright probe (removed afterward) confirmed, after `serviceWorker.controller` becomes non-null: `ipad-slicer-app-<buildId>` contains `index.html`, every JS/CSS chunk, the catalog entry points, `manifest.webmanifest`, `engine-manifest.json` and all icons — i.e. `installShellVersioned` behaves as designed against the real Cache API, not just fakes. |
| E2E (ipad-webkit) | Connectivity/offline scenarios: "an uncached printer selected while offline explains it needs a connection" and "losing and regaining connectivity updates the visible offline status" both pass. "the app shell loads cross-origin isolated" (pre-existing) still passes. |
| E2E gap | Two scenarios (`test.fixme`, not deleted): "a fully cached reload stays cross-origin isolated while offline" and "a printer and engine used online once keep slicing fully offline after reload". Both need `page.reload()`/`page.goto()` while `context.setOffline(true)`. On this machine (Playwright 1.55.1, WebKit, Windows) that combination fails with "WebKit encountered an internal error" **even with zero service worker involved** (confirmed with a throwaway control test: `setOffline(true)` then `goto()` on a page with no SW at all fails identically) — a driver-level limitation, not an application defect. The underlying behavior is covered instead by: `installShellVersioned`'s atomicity tests, the real-cache-contents probe above, and `sw-runtime.test.ts`'s navigate-fallback test. |
| Full `shell.spec.ts` run (both projects) | `npx playwright test tests/e2e/shell.spec.ts --project=ipad-webkit --project=ipad-webkit-landscape`: first run 57 passed / 7 failed / 4 skipped (the two fixme's × 2 projects). Two regressions found and fixed (see below), re-verified individually: exit 0, 6/6. One failure (`a continuous ring drag sweeping past 180 degrees...`, both projects) is **pre-existing and unrelated**: `src/viewer/gestures.ts` is mid-edit by concurrent work elsewhere in this tree (confirmed via `git diff --stat`, 13 insertions/5 deletions, a file this work unit never touches) — not rerun or fixed here, out of scope (`src/viewer/**` is prohibited for this work unit). |
| Regression 1 (found + fixed) | Two pre-existing tests that simulate an engine load failure via `page.route('**/engine/**', route => route.abort(...))` started failing once the service worker was added: Playwright's `page.route()` never sees a request re-issued from *inside* a service worker's own fetch handler (documented Playwright limitation), so the abort no longer reached the now-SW-intercepted engine fetch. Fixed by scoping `test.use({ serviceWorkers: 'block' })` to just those two tests (`tests/e2e/shell.spec.ts`, new `test.describe('engine failure simulation (service worker disabled)', …)`), which is exactly what they need: no SW at all, so the old direct-fetch simulation works again. Reverified: both pass on both projects. |
| Regression 2 (found + fixed) | One pre-existing test (`configuration stays bounded…`, landscape) timed out clicking "Import preset": the `UpdateToast` was visibly open and intercepting pointer events. Root cause: `registration.addEventListener('updatefound', …)` also fires for a page's very first install (there is no previous version to update *from* yet); checking `navigator.serviceWorker.controller` only at 'installed' time is not reliable, because that controller can become non-null moments later from this same registration's own first activation, which then reads as "an update finished installing". Fixed in `src/pwa/register.ts` by snapshotting `wasAlreadyControlled` at 'updatefound' time instead of re-checking the controller at 'installed' time. Reverified: passes on both projects; full `src/pwa` and full Vitest suites stay green (48/48, 441/441). |
| Rollback boundary | Revert `src/pwa/**`, `src/app/stores/connectivity.{ts,test.ts}`, the connectivity/update wiring in `AppProvider.tsx`/`App.tsx`, the `offlinePack` addition in `configuration.ts`, the `pwa.*`/`app.offline`/`configuration.offlinePack` i18n keys, the `swPlugin()` line in `vite.config.ts`, the icon files, and the manifest/`_headers`/`index.html` edits; unrelated concurrent viewer/gizmo work in progress elsewhere in the tree is untouched. |

- Deviations from the literal task wording: pure SW logic lives in `src/pwa/sw-runtime.ts` (imported by `src/pwa/sw.ts`) rather than directly in `sw.ts`, because `self.addEventListener`-based code cannot be unit-tested directly — this matches the build-instruction's own two-step description ("pure, testable routing/isolation helpers first... then the actual service worker logic") and keeps `sw.ts` itself thin. `engine-<release>`/`catalog-<orcaTag>` from design.md became stable-named `ipad-slicer-engine`/`ipad-slicer-catalog`: the release/tag is already embedded in every cached URL (immutable paths), so a single stable cache name is equivalent and simpler; activation therefore only prunes superseded `ipad-slicer-app-*` shell generations, not engine/catalog entries.
- Known gaps / follow-ups: task 11.6 (DiagnosticsSheet storage estimates, tier/probe/crash reasons) not started. Task 11.9 (physical device / real Cloudflare Pages headers) needs explicit deployment authorization and a device — not run. `scripts/serve-dist.mjs` always sends `Cache-Control: no-store` regardless of `public/_headers` (a pre-existing, documented limitation), so the `immutable`-vs-`no-cache` header split added to `_headers` could not be verified against a real HTTP response locally; it was reviewed by hand against the file instead. Icons are functional placeholders, not final art.
