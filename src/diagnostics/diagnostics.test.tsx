...
const labels: DiagnosticsLabels = { heading: 'Diagnostics', isolation: 'Isolation', variant: 'Variant', auto: 'Auto', st: 'Single', mt: 'Multi', unavailableMt: 'Unavailable', export: 'Export', recent: 'Recent', load: 'Load', slice: 'Slice', heap: 'Heap' };
...
it('exports persisted entries and refuses mt when the probe gate fails', async () => {
  const host = document.createElement('div'), reload = vi.fn(), exportJson = vi.fn<(json: string, name: string) => Promise<void>>(async () => undefined); document.body.append(host);
  ...
  dispose = render(() => <DiagnosticsSheet labels={labels} storage={storage} entries={() => [{ ts: 1, type: 'engine-error', data: { message: 'boom' } }]}
    probe={() => ({ variant: 'st', reason: 'no isolation' })} reload={reload} exportJson={exportJson} />, host);
  ...
  host.querySelector('button')!.click(); await Promise.resolve();
