import type { Dictionary } from './en';

export const es = {
  app: {
    title: 'SliceAr',
    stepsLabel: 'Pasos de laminado',
    engine: 'Motor',
    modelRequired: 'Importa un modelo antes de laminar.',
    configurationRequired: 'Elige una impresora, un filamento y una calidad compatibles antes de laminar.',
    offline: 'Sin conexión. Las impresoras, presets y laminados en caché siguen funcionando.',
  },
  pwa: {
    updateMessage: 'Hay una nueva versión disponible.',
    update: 'Actualizar',
    dismiss: 'Ahora no',
  },
  steps: {
    import: 'Importar',
    configure: 'Configurar',
    preview: 'Vista previa',
    save: 'Guardar',
  },
  panes: {
    import: 'Agrega el modelo que quieres imprimir. Se admiten archivos STL y 3MF.',
    configure: 'La impresora, el material y la calidad se configuran aquí. El modo simple es el predeterminado.',
    preview: 'La placa y, después de laminar, la vista previa aparecen aquí.',
    save: 'Guarda el G-code en Archivos o compártelo.',
  },
  import: {
    heading: 'Importar un modelo',
    upload: 'Subir desde el dispositivo',
    dropTitle: 'Suelta aquí archivos STL o 3MF',
    dropActive: 'Suelta para agregar los archivos',
    formats: 'Formatos admitidos: STL y 3MF.',
    limits: '{count}/{limit} objetos en la placa. Hasta {triangles} triángulos en total.',
    importing: 'Leyendo el archivo…',
    addedOne: 'Modelo agregado a la placa.',
    addedMany: '{count} modelos agregados a la placa.',
    continue: 'Continuar a Configurar',
    sourcesHeading: 'Obtener modelos',
    sourcesNote: 'Descarga un STL o 3MF allí y luego súbelo aquí.',
    licenseNote: 'Cada modelo conserva su propia licencia. Revisa las condiciones antes de imprimirlo o compartirlo.',
    opensNewTab: 'se abre en una pestaña nueva',
  },
  results: {
    time: 'Tiempo de impresión', mass: 'Masa de filamento', cost: 'Costo estimado', layers: 'Capas', unavailable: 'No disponible',
    priceBasis: 'Precio del filamento', requested: 'Ajustes solicitados', effective: 'Ajustes efectivos de salida', stale: 'Los ajustes o la placa cambiaron; volvé a laminar antes de confiar en este resultado.',
    slicing: 'Laminando', preparing: 'Preparando motor', ready: 'Motor listo', save: 'Guardar G-code',
  },
  preview: {
    canvas: 'Vista previa de la trayectoria del G-code', layer: 'Capa', height: 'Altura', previousLayer: 'Capa anterior', nextLayer: 'Capa siguiente',
    layerSlider: 'Control de capas', loading: 'Cargando vista previa…',
    tooLarge: 'La vista previa es demasiado grande para este dispositivo. Las estimaciones y Guardar siguen funcionando.',
    webglUnavailable: 'La vista previa 3D necesita WebGL, que no está disponible aquí. Las estimaciones y Guardar siguen funcionando.',
    loadFailed: 'No se pudo cargar la vista previa. Las estimaciones y Guardar siguen funcionando.',
    reduced: 'G-code grande: la vista previa muestra solo las capas cercanas a la seleccionada para respetar el límite de memoria.',
    optionsToggle: 'Opciones de vista previa',
    colorMode: 'Color por',
    colorModeSingle: 'Color único', colorModeFeature: 'Tipo de trazo', colorModeFeedrate: 'Velocidad',
    colorModeLayerHeight: 'Altura', colorModeObject: 'Por objeto', colorModeTool: 'Por herramienta',
    colorModeFilament: 'Por filamento', colorModeColorChange: 'Por cambio de color', colorModeMoveKind: 'Tipo de movimiento',
    colorModePower: 'Potencia de herramienta',
    featureLegend: 'Referencia de trazos',
    featurePerimeter: 'Perímetro', featureExternalPerimeter: 'Perímetro exterior', featureInfill: 'Relleno',
    featureSolidInfill: 'Relleno sólido', featureSupport: 'Soporte', featureSkirt: 'Falda', featureBrim: 'Borde',
    featureBridge: 'Puente', featureTravel: 'Desplazamiento', featurePrimeTower: 'Torre de purga', featureWipeTower: 'Torre de limpieza',
    featureRaft: 'Balsa', featurePurge: 'Purga',
    declutter: 'Ocultar de la vista',
    showTravel: 'Movimientos de desplazamiento', showWipe: 'Movimientos de limpieza', showRetractions: 'Marcas de retracción',
    viewPresets: 'Vistas de cámara', viewFit: 'Ajustar', viewTop: 'Superior', viewFront: 'Frontal', viewIso: 'Iso',
    saveImage: 'Guardar imagen de vista previa', saveImageBusy: 'Guardando imagen…',
    estimatedTime: 'Tiempo de impresión estimado', kinematicNote: 'aproximado, no es la estimación propia del laminador',
  },
  diagnostics: {
    heading: 'Diagnósticos', isolation: 'Aislamiento de origen cruzado', variant: 'Variante del motor', st: 'Un hilo', mt: 'Multihilo',
    unavailableMt: 'El modo multihilo no está disponible en este dispositivo.', retryMt: 'Reintentar multihilo', retryMtSuccess: 'El reintento multihilo está habilitado.',
    export: 'Exportar diagnósticos', recent: 'Eventos recientes', load: 'Carga del motor', slice: 'Tiempo de laminado', heap: 'Memoria máxima',
  },
  preferences: {
    heading: 'Preferencias',
    language: 'Idioma',
    theme: 'Apariencia',
    performance: 'Rendimiento',
    english: 'Inglés',
    spanish: 'Español',
    system: 'Sistema',
    light: 'Claro',
    dark: 'Oscuro',
    auto: 'Automático',
    standard: 'Estándar',
    full: 'Completo',
    activeTier: 'Nivel activo',
  },
  viewer: {
    workspace: 'Espacio de trabajo del modelo', importModel: 'Importar modelo', buildPlate: 'Placa de impresión 3D', plateObjects: 'Objetos de la placa', objectCount: '{count}/{limit} objetos',
    toolbar: 'Herramientas del objeto seleccionado',
    history: 'Historial de deshacer',
    mode: 'Modo de transformación', select: 'Seleccionar', move: 'Mover', rotate: 'Girar',
    snap: 'Ajuste',
    viewPresets: 'Vista de cámara', viewFit: 'Ajustar', viewTop: 'Superior', viewFront: 'Frontal', viewIso: 'Iso',
    models: 'Modelos', modelsList: 'Modelos en la placa',
    modelsHide: 'Ocultar {name}', modelsShow: 'Mostrar {name}', modelsMenu: 'Más acciones para {name}',
    modelsDuplicate: 'Duplicar {name}', modelsDelete: 'Eliminar {name}', modelsRename: 'Renombrar {name}', modelsRenamePrompt: 'Nuevo nombre',
    deselect: 'Deseleccionar',
    layFlat: 'Apoyar en una cara',
    layFlatPending: 'Esta función se habilitará después de integrar el motor en la fase 7.',
    autoOrient: 'Auto-orientar',
    autoOrientNoFreeSpot: 'La auto-orientación colocó el objeto en el centro de la placa; no se encontró un lugar totalmente libre.',
    rotateX: 'Girar 90° en X',
    rotateY: 'Girar 90° en Y',
    scale: 'Escalar',
    duplicate: 'Duplicar',
    delete: 'Eliminar',
    reset: 'Restablecer',
    undo: 'Deshacer',
    redo: 'Rehacer',
    scaleTitle: 'Tamaño del objeto',
    close: 'Cerrar',
    size: 'Dimensión mayor',
    unit: 'Unidad de tamaño',
    suspiciousSize: 'Este tamaño parece inusual. ¿El archivo de origen se exportó en otras unidades?',
    multiply25_4: '×25,4',
    multiply1000: '×1000',
    divide10: '÷10',
    keepEntered: 'Conservar valor ingresado',
    resizeSheet: 'Cambiar tamaño del panel de escala',
    editValues: 'Editar valores',
    transformTitle: 'Transformación del objeto',
    positionHeading: 'Posición (mm)',
    positionX: 'Posición X', positionY: 'Posición Y', positionZ: 'Posición Z',
    rotationHeading: 'Rotación (°)',
    rotationX: 'Rotación X', rotationY: 'Rotación Y', rotationZ: 'Rotación Z',
    scaleHeading: 'Escala (%)',
    scaleX: 'Escala X', scaleY: 'Escala Y', scaleZ: 'Escala Z',
    scaleLink: 'Mantener proporcional la escala X, Y y Z',
    dimensionsHeading: 'Dimensiones (mm)',
    dimensionX: 'Ancho (X)', dimensionY: 'Profundidad (Y)', dimensionZ: 'Alto (Z)',
    resizeTransformSheet: 'Cambiar tamaño del panel de transformación',
  },
  configuration: {
    arrangeComplete: 'La placa fue orientada y organizada.',
    search: 'Buscar impresoras', recommended: 'Recomendadas', verifiedToSlice: 'Verificadas para laminar', noPrinters: 'Ninguna impresora coincide con la búsqueda.', printer: 'Impresora', filament: 'Filamento', quality: 'Calidad', loading: 'Cargando impresora…', unavailable: 'No disponible',
    draft: 'Borrador', standard: 'Estándar', fine: 'Fina', simple: 'Simple', advanced: 'Avanzado', arrange: 'Orientar y organizar',
    arrangePending: 'Orientar y organizar estará disponible después de importar un modelo.', estimates: 'Tiempo, filamento y costo estimados', reset: 'Restablecer al preset',
    plateWide: 'Los ajustes de soporte y borde se aplican a toda la placa. Los controles por objeto no están disponibles.', notSmokeTested: 'Impresora del usuario — sin prueba de humo individual',
    slice: 'Laminar', cancel: 'Cancelar laminado', finishing: 'Finalizando el laminado anterior', sliceComplete: 'Laminado completo (capas)', incomplete: 'Seleccione una impresora, un filamento y una calidad compatibles antes de laminar.', settingsMode: 'Modo de ajustes', invalidSetting: 'Ajuste inválido', selectPrinterFirst: 'Seleccione una impresora antes de importar un preset.',
    profileUnavailable: 'El perfil de la impresora no está disponible. Inténtelo nuevamente.', offlinePack: 'Esta impresora necesita conexión para descargarse. Inténtelo de nuevo cuando vuelva a estar en línea.', invalidCustom: 'La impresora personalizada no es válida y no se guardó.', invalidCustomFilament: 'El filamento personalizado no es válido y no se guardó.', invalidImport: 'No se pudo importar el preset; se conservaron los ajustes actuales.',
    imported: 'Preset importado.', importedWithNotices: 'Preset importado. Se omitieron los ajustes no compatibles.', customSaved: 'Impresora personalizada guardada. Seleccione un filamento y una calidad compatibles para continuar.', customFilamentSaved: 'Filamento personalizado guardado y seleccionado.',
    categories: { quality: 'Calidad', strength: 'Resistencia', speed: 'Velocidad', support: 'Soporte', others: 'Otros' },
    custom: {
      heading: 'Impresora personalizada', name: 'Nombre', width: 'Ancho de cama (mm)', depth: 'Profundidad de cama (mm)', height: 'Altura imprimible (mm)', nozzle: 'Diámetro de boquilla (mm)', flavor: 'Formato de G-code', start: 'G-code inicial', end: 'G-code final', heated: 'Cama caliente', save: 'Guardar impresora personalizada',
      filament: { heading: 'Filamento personalizado', name: 'Nombre del filamento', base: 'Filamento base', save: 'Guardar filamento personalizado', disclaimer: 'Filamento del usuario — sin prueba de humo individual' },
    },
    presets: { heading: 'Transferencia de presets', json: 'JSON del preset', import: 'Importar preset', export: 'Exportar preset actual' },
  },
  settings: {
    layerHeight: { label: 'Altura de capa' }, initialLayerHeight: { label: 'Altura de capa inicial' },
    wallLoops: { label: 'Paredes' }, topLayers: { label: 'Capas superiores' }, bottomLayers: { label: 'Capas inferiores' },
    infillDensity: { label: 'Densidad de relleno' }, infillPattern: { label: 'Patrón de relleno' }, supports: { label: 'Soportes' },
    supportType: { label: 'Tipo de soporte' }, supportAngle: { label: 'Ángulo de soporte' }, buildPlateOnly: { label: 'Solo en la placa' },
    brimType: { label: 'Tipo de borde' }, brimWidth: { label: 'Ancho del borde' }, bedType: { label: 'Placa de impresión' },
    nozzleTemperature: { label: 'Temperatura de boquilla' }, initialNozzleTemperature: { label: 'Temperatura inicial de boquilla' },
    bedTemperature: { label: 'Temperatura de cama' }, initialBedTemperature: { label: 'Temperatura inicial de cama' },
    nozzleDiameter: { label: 'Diámetro de boquilla' }, printableHeight: { label: 'Altura imprimible' }, printableArea: { label: 'Área imprimible' },
    outerWallSpeed: { label: 'Velocidad de pared exterior' }, innerWallSpeed: { label: 'Velocidad de pared interior' }, infillSpeed: { label: 'Velocidad de relleno' },
    travelSpeed: { label: 'Velocidad de desplazamiento' }, initialLayerSpeed: { label: 'Velocidad de capa inicial' }, relativeExtrusion: { label: 'Extrusión relativa' },
    ironingType: { label: 'Planchado' }, seamPosition: { label: 'Posición de costura' }, fuzzySkin: { label: 'Textura rugosa' },
    zHop: { label: 'Altura de elevación Z' }, zHopType: { label: 'Tipo de elevación Z' },
    gcodeFlavor: { label: 'Formato de G-code' }, beforeLayerGcode: { label: 'G-code previo a la capa' }, layerChangeGcode: { label: 'G-code de cambio de capa' },
    startGcode: { label: 'G-code inicial' }, endGcode: { label: 'G-code final' }, filamentCost: { label: 'Costo del filamento' }, filamentDensity: { label: 'Densidad del filamento' },
    options: {
      supportType: {
        'normal(auto)': 'Normal (automático)', 'tree(auto)': 'Árbol (automático)',
        'normal(manual)': 'Normal (manual)', 'tree(manual)': 'Árbol (manual)',
      },
      brimType: {
        auto_brim: 'Automático', brim_ears: 'Solo en las esquinas', painted: 'Pintado (manual)', outer_only: 'Solo borde exterior',
        inner_only: 'Solo borde interior', outer_and_inner: 'Bordes exterior e interior', no_brim: 'Ninguno',
      },
      infillPattern: {
        rectilinear: 'Líneas rectas', alignedrectilinear: 'Líneas rectas alineadas', zigzag: 'Zigzag', 'zig-zag': 'Zigzag', crosszag: 'Zigzag cruzado',
        lockedzag: 'Zigzag bloqueado', line: 'Líneas', grid: 'Cuadrícula', triangles: 'Triángulos', 'tri-hexagon': 'Tri-hexágono',
        cubic: 'Cúbico', adaptivecubic: 'Cúbico adaptativo', quartercubic: 'Cúbico a un cuarto', supportcubic: 'Cúbico de soporte',
        lightning: 'Rayo (rápido y liviano)', honeycomb: 'Panal de abeja', '3dhoneycomb': 'Panal 3D',
        'lateral-honeycomb': 'Panal lateral', 'lateral-lattice': 'Retícula lateral', crosshatch: 'Entramado cruzado',
        tpmsd: 'TPMS-D', tpmsfk: 'TPMS-FK', gyroid: 'Giroide', concentric: 'Círculos concéntricos',
        hilbertcurve: 'Curva de Hilbert', archimedeanchords: 'Cuerdas de Arquímedes', octagramspiral: 'Espiral octagonal',
      },
      bedType: {
        'Cool Plate': 'Placa fría', 'Engineering Plate': 'Placa de ingeniería', 'High Temp Plate': 'Placa de alta temperatura',
        'Textured PEI Plate': 'Placa PEI texturizada', 'Textured Cool Plate': 'Placa fría texturizada', 'Supertack Plate': 'Placa Supertack',
      },
      gcodeFlavor: { marlin: 'Marlin', klipper: 'Klipper', reprapfirmware: 'RepRapFirmware', repetier: 'Repetier', marlin2: 'Marlin 2' },
      ironingType: { 'no ironing': 'Ninguno', top: 'Superficie superior', topmost: 'Solo la última capa', solid: 'Todas las superficies sólidas' },
      seamPosition: { nearest: 'Punto más cercano', aligned: 'Alineada', aligned_back: 'Alineada atrás', back: 'Atrás', random: 'Aleatoria' },
      fuzzySkin: {
        none: 'Desactivado', external: 'Paredes exteriores', hole: 'Solo agujeros', all: 'Todas las paredes',
        allwalls: 'Todas las paredes (con superior e inferior)', disabled_fuzzy: 'Deshabilitado',
      },
      zHopType: {
        'Auto Lift': 'Automático', 'Normal Lift': 'Normal (recto)', 'Slope Lift': 'Rampa', 'Spiral Lift': 'Espiral',
      },
    },
  },
  errors: {
    'viewer-start-failed': 'No se pudo iniciar el visor 3D.',
    'stl-empty': 'El archivo STL está vacío o es demasiado corto.',
    'stl-no-triangles': 'El archivo STL no contiene triángulos válidos.',
    'stl-invalid-normals': 'El archivo STL no contiene normales de vértice válidas.',
    'stl-unsupported-buffers': 'El archivo STL produjo búferes de malla no compatibles.',
    'stl-invalid-numbers': 'El archivo STL contiene valores numéricos no válidos.',
    'stl-invalid-bounds': 'No se pudieron calcular los límites del archivo STL.',
    'stl-invalid-format': 'No se pudo analizar el archivo STL.',
    'stl-read-failed': 'No se pudo leer el archivo STL.',
    'worker-failed': 'No se pudo iniciar el proceso de importación STL.',
    'worker-unreadable': 'El proceso de importación STL devolvió una respuesta ilegible.',
    'tier-limit-objects': 'Este nivel de rendimiento admite como máximo {limit} objetos.',
    'tier-limit-triangles': 'Este nivel de rendimiento admite como máximo {limit} triángulos.',
    'prepare-configuration-missing': 'Elige una impresora, un filamento y una calidad compatibles antes de preparar la placa.',
    'prepare-model-missing': 'Importa al menos un modelo antes de preparar la placa.',
    'mesh-missing': 'Falta la malla {meshId}',
    'invalid-preparation-result': 'El resultado de preparación no es válido',
    'invalid-slice-result': 'El resultado de laminado no es válido',
    'threemf-not-zip': 'El archivo 3MF no es un archivo 3MF válido.',
    'threemf-no-model': 'El archivo 3MF no contiene una parte de modelo 3D.',
    'threemf-no-triangles': 'El archivo 3MF no contiene triángulos imprimibles.',
    'threemf-unsupported': 'Este archivo 3MF está cifrado o depende de partes que no están incluidas, por lo que no se puede importar.',
    'threemf-invalid-format': 'No se pudo analizar el archivo 3MF.',
    'threemf-too-large': 'El archivo 3MF es demasiado grande para importarlo en este dispositivo.',
  },
} satisfies Dictionary;
